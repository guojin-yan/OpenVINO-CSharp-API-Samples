    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                if (!File.Exists("./model/yolov6s.bin") && !File.Exists("./model/yolov6s.bin"))
                {
                    if (!File.Exists("./model/yolov6s.tar"))
                    {
                        _ = Download.download_file_async("https://github.com/guojin-yan/OpenVINO-CSharp-API-Samples/releases/download/Model/yolov6s.tar",
                            "./model/yolov6s.tar").Result;
                    }
                    Download.unzip("./model/yolov6s.tar", "./model/");
                }
            ...
            yolov6_det(model_path, image_path, device);
            //yolov7_det_with_process(model_path, image_path, device);
        }
        static void yolov6_det(string model_path, string image_path, string device)
        {
            ...
            start = DateTime.Now;
            Mat result_data = new Mat(8400, 85, DepthType.Cv32F, 1,
                           Marshal.UnsafeAddrOfPinnedArrayElement(output_data, 0), 4 * 85);
            ...
            for (int i = 0; i < result_data.Rows; i++)
            {
                ...
                Mat classes_scores = new Mat(result_data, new Rectangle(5, i, 80, 1));//GetArray(i, 5, classes_scores);
        ...
        static void yolov6_det_with_process(string model_path, string image_path, string device)
        {
            DateTime start = DateTime.Now;
            // -------- Step 1. Initialize OpenVINO Runtime Core --------
            Core core = new Core();
